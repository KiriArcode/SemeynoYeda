...
## ⚠️ Важно: Применение миграций

Перед загрузкой seed-данных убедитесь, что все миграции применены к базе данных Neon. См. [NEON_MIGRATIONS.md](./NEON_MIGRATIONS.md) для инструкций по применению миграций.

**Последняя миграция:** `20250211000001_wabba_recipe_fields.sql` — добавляет поля `wabba_ratings` и `excluded_from_menu` в таблицу `recipes`.
...
При добавлении рецептов через скрипт seed или через компонент импорта JSON система автоматически проверяет дубликаты:

- **По названию (title)**: если рецепт с таким названием уже существует
- **По slug**: если рецепт с таким slug уже существует

Если обнаружен дубликат:
- При использовании `seedNeon.ts` - рецепт будет пропущен (используется `ON CONFLICT`)
- При использовании компонента импорта JSON - рецепт будет показан в списке с пометкой "Дубликат" и не будет добавлен

**Рекомендация**: перед массовым импортом проверьте существующие рецепты, чтобы избежать дублирования.
