...
interface Recipe {
  ...
  // Wabba — оценки рецептов
  wabbaRatings?: WabbaRatings;   // оценки Коля/Кристина (свайп вправо/влево)
...
## Структура WabbaRatings

```typescript
type WabbaRating = 'like' | 'dislike';

interface WabbaRatings {
  kolya?: WabbaRating;
  kristina?: WabbaRating;
}
```

Используется для хранения оценок рецептов через интерфейс Wabba (`/wabba`). После того, как оба участника оценили рецепт, автоматически обновляется `suitableFor` или устанавливается `excludedFromMenu`.
...
- `wabbaRatings?: WabbaRatings` — оценки участников (Коля/Кристина) через свайп в приложении
- `excludedFromMenu?: boolean` — рецепт исключён из меню (оба участника свайпнули влево)

### Логика оценок

После того, как оба участника оценили рецепт:

- **Коля вправо + Кристина влево** → `suitableFor: 'kolya'`
- **Коля влево + Кристина вправо** → `suitableFor: 'kristina'`
- **Оба вправо** → `suitableFor: 'both'`
- **Оба влево** → `excludedFromMenu: true` (рецепт не участвует в генерации/планировании меню)

### Где используются

1. **WabbaPage** (`/wabba`) — экран оценки рецептов за последние 2 недели
2. **SwapModal** — рецепты с `excludedFromMenu: true` не показываются при замене блюда
3. **Генерация меню** — исключённые рецепты не участвуют в автоматическом подборе
...
**Wabba оценки:**
- Добавляются автоматически через интерфейс Wabba (`/wabba`)
- Рецепты за последние 2 недели показываются для оценки
- После оценки обоими участниками автоматически обновляется `suitableFor` или `excludedFromMenu`
